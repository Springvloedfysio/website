(function($) {
    ...
    var defopts = {
        title_text      : "this link opens a new window to '{title}'",
        favicon         : true,
    ...
    var methods = {
        init        : function(opts) {
            ...
            var manipFn = 'prependTo', // manipulation method to use when injecting the favicon span elements
                ...
                finks   = [],
                furls   = [],
                items   = (typeof opts.link_selector === 'string') && opts.link_selector.length ? this.filter(opts.link_selector) : this
                        ;

            if ('right' === opts.span_position)
                manipFn = 'appendTo';

            var setTitleText = (function(txt) {
                if (!txt || typeof txt !== 'string' || !txt.length)
                    return function(o) {};
                if (txt.match(/\{title\}/))
                    return function(o) { o.attr('title', txt.replace('{title}', (o.attr('title') || o.attr('href')))); };

                return function(o) { o.attr('title', txt); };
            })(opts.title_text);
        ...
        destroy     : function() {
            return this.each(function(n) {
                ...
                $this.attr('title',  opts.orig_title  ? opts.orig_title  : null);
            });
        }
    };
